...
/* Props
*/
import type { TabsProps, TabsListProps } from '@radix-ui/react-tabs'
...
type PickerProp<Array extends ReadonlyArray<unknown>> = TabsProps & ListBuilder<Array> &
...
export const Picker: <const Array extends readonly string[]>(prop: PickerProp<Array>) => ReturnType<typeof Root> = (prop) => {
	...
	return (
		<Root className='space-y-2.5' onValueChange={onValueChange}>
			{
				list(
					{
						children: tags.map(
							(tag) => {
								...
								return (
									<Fragment key={tag} children={component(tag)} />
								)
							}
						)
					}
